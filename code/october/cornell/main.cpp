#include <../utils/utils/bsdf_sampler.h>
...
int main()
{
    ...
    auto const trace = [&](Ray const cameraRay) noexcept
    {
            ...
            -> vec3
        {
            ...
            return lightSample.transform
            (
                [&](Scene::SurfacePointSample const &sample) noexcept
                {
                    auto const &[q0, pdf] = sample;
                    f32 const pdfBSDF = bsdfSampler.pdfW(q0, q1, q2);
                    return pdf / (pdf + pdfBSDF) * bsdfSampler.BSDF(q0, q1, q2) * lightSampler.emission(q0, q1) / pdf;
                }
            ).value_or(vec3(0.f))
            +
            bsdfSample.transform
            (
                [&](Scene::SurfacePointSample const &sample) noexcept
                {
                    auto const &[q0, pdf] = sample;
                    f32 const pdfLS = lightSampler.pdfW(q0, q1, q2);
                    return pdf / (pdf + pdfLS) * bsdfSampler.BSDF(q0, q1, q2) * (lightSampler.emission(q0, q1) + L(q0, q1)) / pdf;
//                    return lightSample + pdf / (pdf + pdfLS) * bsdfSampler.BSDF(q0, q1, q2) * (lightSampler.emission(q0, q1) + L(q0, q1)) / pdf;
                }
            ).value_or(skyL);
        };
